...
- **開発基盤: ruff の設定をチームコンテナの実配線に合わせて再構成**
  ...
  配置し、直下 `ruff.toml` は extend で継承する薄い配線に縮小。免除は各ファイル
  内の抑止コメント（ルール名表記・理由つき。ファイル単位は
  `# ruff: file-ignore[...]`）へ全面移設 — チームコンテナのエディタ・/check は
  リポジトリ側の toml を読まないため、ファイルに同居する抑止だけが全環境で
  同じに効く。CI の lint は `--preview` 一本に変更（チームの関門と同基準）。
  合わせて実害なく直せる違反 20 件を修正（テストの未使用 fixture 引数の削除・
  bool 引数のキーワード化、ui のループ中削除の複製反復化・新リスト作成の
  `[*x, y]` 化ほか。挙動・画面は不変、テスト 339 件全パス）
...
から不変(削除した表記・関数はいずれも使用実績なし)。**互換性注意**: 未使用の
公開関数(io_jsonc の save 系ほか)と開発初期の表記の受理を削除(各項参照)。

- **欠けファイル・gz 圧縮まわりの修正**（2026-07-31。処理フロー図解の作成時に
  コードを網羅的に読んで発見した潜在不具合の一掃）:
  - `dataName_{type}.csv` が無いのに DataName 軸を要求した場合、存在確認なしの
    scan による polars の FileNotFoundError（パーツ名の名指しなし・全件収集にも
    捕まらない型）ではなく、parameterLabel_ 側と同じ「解決できない軸」の
    軸名つき ValueError になるよう修正（axis_resolve）
  - introspect（UI 向けの type 検出・軸カタログ・Measure ラベル）が
    `.csv.gz` も読むように（エンジン `data_file` と同じ扱い）。gz だけの
    ディレクトリで「エンジンは計算できるのに UI では type が出ない」非対称を解消
- **開発初期の自前表記の互換コードを削除**（2026-08-03。リリース実績が無く、
  実験側の config（sample.jsonc / reference_scripts）には一度も存在しなかった
  表記のため互換不要とユーザー確認）: `values`（`value` の別名）、
  `*_subset` op（mean_subset 等）、`group_reduce` op の案内エラー、
  `relative.enabled` ガード。対応テストと testing_guide の監査メモも削除
- **呼称の統一**: WLgroup / WLgroupDefinLogical / WLgroupWeight は実験スクリプト
  が読む config の正式な形式であり「旧形式キー」という呼び方は誤りだったため、
  コード内コメント・docs 全体で「WLgroup 系キー」に統一
  （`test_wlgroup_legacy.py` → `test_wlgroup_keys.py` に改名。挙動は不変）
- **リリース前監査での未使用・冗長コードの削除**（2026-08-03。エンジン・UI・
  ファイルの3方向監査 — 全候補の参照ゼロを確認の上で実施、計算結果・表示は不変）:
  - 呼び出しゼロの公開関数を削除: `io_jsonc.save_run_config` /
    `load_score_file` / `save_score_file`、`jsonc.dumps`（利用実績なし —
    互換性注意: これらを import していた外部コードがあれば load 系のみ残る）
  - 重複実装の一本化: custom_parts ローダを cli / batch で1実装に、
    `aggregate._SIMPLE_OPS` を `models.MULTI_OPS` から導出、`_is_weight` の
    数値判定を `_is_number` に統一（op 追加時の二重更新事故を予防）
  - 効果の無い遅延 import 4箇所をトップレベルへ（import 連鎖でロード済みの
    もののみ。polars スレッド数対策等の実効性ある遅延 import は維持）
- UI（次の ui-YYYYMMDD 節の材料。エンジン版数は動かさない）:
  - 単一ユーザ時代の下書きフォールバック（`DRAFT_PATH` と旧形式の受け入れ）を
    削除 — 下書きパスは常に `draft_path_for(ユーザ名)` で解決（サーバ運用
    実績なしのため互換不要とユーザー確認）
  - context の書くだけキー `types` を削除（`part_types` に一本化）
  - 画面2の1描画中に3回走っていた全パーツの値照合を1回にし、undo 用
    スナップショットも再利用（表示・保存内容は不変）
  - 「既存スコア設定を読み込む」の冗長なグループ定義取り込みを削除
    （`to_score_file()` が統合済みのため no-op だった — 監査で「引数欠けの
    バグ疑い」として検出されたが、テストで実害なし・冗長と確定し、
    WLgroupDefinLogical / WLgroupWeight が保たれる固定テストを追加）
  - リポジトリ整理: 対話メモ `score_gui.md` を追跡解除、`docs/flow_guide.html`
    を追跡、.gitignore を強化（`result_tmp*` のファイル形・私物 config・
    `docs/assets/` の受け皿）
  - データに無い type のパーツで「雛形を再生成」を押すと例外画面（KeyError）に
    なっていたのを、ボタンの無効化に修正（他 type のカタログで代替すると誤った
    雛形が黙ってできるため代替はしない）
  - 軸カタログが空のパーツ（設定のみ編集）で「+ 事前集計を追加」を押すと
    例外画面になっていたのを、ボタンの無効化に修正
  - グループ定義・重みセット・制約だけを編集したセッションが下書きに自動保存
    されなかったのを修正（保存対象キーの追加）

## ver.0.7.1 — 2026-07-31

エンジンの数値結果・config(jsonc)の語彙は 0.7.0 から不変。**呼び出し互換と
エラー時の挙動が変わる**(各項の互換性注意を参照): 公開 API の位置渡しは
不可に、欠けデータの計算は「黙って値がズレる」代わりにエラーで止まる。

- **パーツ計算のエラーを全件まとめて報告**（2026-07-31。挙動変更）。従来は1個目の失敗で即座に落ちたため「1つ直して動かしたら
...
- **計算エラーの原因診断+「黙って値がズレる」経路の遮断**（2026-07-31。
...
- **計算エラーが失敗したスコアパーツを常に名指しするように**（2026-07-31。
  ...
  (2026-07-31 実機報告: 候補が実データ由来になったことで、プルダウンの
  ...
  **「元に戻す」を作り直し**(2026-07-31 実機報告: 一部の操作しか戻らない
...
- **UI エクスポート**: WL 軸の "WLgroup" 定義（と WLgroupWeight）を WLgroup 系キー
  （`WLgroup` / `WLgroupDefinLogical` / `WLgroupWeight` — 実験スクリプトが読む
  config の正式な形式）**だけ**に書き出し、groupDefs / weightSets には残さない。
  合成すると実験スクリプトが読む場所がそのまま編集後の内容になり、定義は常に1つ
- **ScoreFile が WLgroup 系キーを解釈**（score.jsonc 単体形式でも読める）。
  groupDefs / weightSets に同名があればそちらが勝つ（RunConfig と同じ優先）
- `RunConfig.to_score_file()` が WLgroup 系キーも groupDefs へ統合（weightSets と
  対称に。エンジン計算は従来から統合済みのため挙動不変 — UI 取り込み経路の
  取りこぼし修正）
- UI: グループ定義名 "WLgroup" は WL 軸の予約名に（WLgroup 系キーは WL への
  定義しか表現できないため WL 以外の軸には使えない）
- **互換性**: 合成後 config は昔からある語彙のみなので旧エンジンでも同じに
  動く。旧エンジンが**新しい score.jsonc 単体**を読んだ場合のみ WLgroup が
  読み飛ばされ、参照パーツが明示エラーになる（静かな誤計算にはならない）。
  以前のエクスポート形（groupDefs に WLgroup 入り）の score.jsonc は引き続き読める
