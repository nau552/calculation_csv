# Copyright (c) 2026
"""Measure 番号 / DataName による相対化・filter のエンドツーエンドテスト(docs/spec_change_dataname_measure.md)。
...
def _measure_part(**relative_overrides: dict[str, str]) -> ScorePart:
...
def test_measure_split_equals_override_split(data_dir_mini: Path) -> None:
    """Measure 分割が Read_Override 分割と同じスコアになることを検証する。"""
    assert math.isclose(
...
def test_dataname_split_equals_measure_split(data_dir_mini: Path) -> None:
    """DataName 分割が Measure 分割と同じスコアになることを検証する。"""
...
def test_measure_split_with_labels_annotation(data_dir_mini: Path) -> None:
    """Labels 注記は計算に影響しない。"""
    p = _measure_part(labels={"1": "evaluation_param_read_level_1", "0": "reference_param_read_level_1"})
    assert math.isclose(
        compute_score_part(data_dir_mini, p),
        compute_score_part(data_dir_mini, _measure_part()),
        rel_tol=1e-12,
...
def test_measure_filter_pins_one_measurement(data_dir_mini: Path) -> None:
    """相対化なしで Measure filter だけでも1測定を選べる(絶対値パーツ)。"""

    def _abs_part(measure: int) -> ScorePart:
        return ScorePart.model_validate(
            {
                "name": f"m{measure}",
                "type": "FBC",
...
def test_measure_filter_is_in_selects_multiple(data_dir_mini: Path) -> None:
    """複数値 filter(is_in): [0, 1] は測定 0 と 1 の行を残し、後段の mean が両者をまとめて畳む。

    単一値2つの平均と一致する(行数が等しい mini データ)。
    """
    p = ScorePart.model_validate(
        {
            "name": "m01",
...
class TestPrefilterWithMeasure:
    ...
    def test_is_in_prefilter_same_value_and_cache_key_hashable(
        ...
        同値性と、shared_ctx のキャッシュキーがリスト値でも壊れない
        (tuple 化される)ことの両方を確認する。
        ...
        p = ScorePart.model_validate(
            {
                ...
                "relative": {
                    "split_axis": "State",
                    "numerator_when": "A2B",
                    "denominator_when": "R2A",
                    "denominator_offset": 1,
                },
                "order": ["Measure", *tail],
                "aggregations": {"Measure": {"op": "filter", "value": [0, 1]}, **{a: {"op": "mean"} for a in tail}},
            }
        )
        assert _hoistable_prefilters(p) == [("Measure", [0, 1])]
