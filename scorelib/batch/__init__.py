...
（config.jsonc）でバッチ単位に一括計算する。エンジン本体（scorelib/*.py）の
単一 epoch 計算とは数値等価（tests/test_batch.py で保証）。

CLI: ``python -m scorelib.batch --config ... --history ... --out scores.csv``
...
読み込んでしまうと効かなくなるため。``from scorelib.batch import X`` は
従来どおり動く。
"""
import importlib
