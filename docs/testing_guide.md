...
| `test_cli.py` | 本丸。fixtures の config を実データで計算し、**テスト内に独立に書いた素朴な再計算と一致**することを照合（FBCパーツ）。遅延グループ集計のユーザシナリオ。範囲外WL値のエラー。custom パーツの計算・エラー・混在拒否（関数不在は TypeError — 2026-07-31 の品質向上パスで ValueError から変更）。**サブプロセスとして CLI を起動する完全E2E**（最適化側から呼ばれる形そのもの） |
