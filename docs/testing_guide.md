...
2. **将来の変更の安全網** — 機能追加やリファクタリングで**別の場所が壊れたら即座に検知**できる。
   このリポジトリは145本のテストを毎回全部回しており、大規模な変更(共有キャッシュ導入・
   op 体系の再設計等)を安心してやれたのはこの網のおかげ
...
| ファイル | 層 | 内容 |
|---|---|---|
| `test_jsonc.py` | 単体 | コメント除去が文字列内の `//` を壊さないこと、末尾カンマ、実際の config.jsonc が読めること（2026-08-03: 未使用だった load/save_score_file のラウンドトリップテストは関数ごと削除） |
| `test_expression.py` | 単体 | 式評価（関数、変数参照、未定義参照のエラー） |
| `test_aggregate.py` | 単体 | 各op（filter / mean系+value による対象限定 / diff / expr）を手計算できる数行のデータで照合。潰し残しエラー。グループ派生列が普通の軸として集計できること |
| `test_axis_resolve.py` | 単体 | 必要な軸だけの結合・map解決・Override の Boolean 化、**dataName_* 欠落時の軸名つきエラー**（存在確認なしの scan で FileNotFoundError が漏れていた回帰 — 2026-07-31） |
...
| `test_introspect.py` | 単体 | type検出（予約ファイル無視・**値列ルール**: ファイル名と同名の値列を持つcsvのみ。Measure列だけでは type にならない）、中身の形による設定/係数jsonc検出、軸カタログ（**実データに存在する値への絞り込み**、tRにStateが漏れない等、Measure/DataName 軸の追加と Measure 列無し type での非表示、Param 候補 [ROM, Opt]、**全行空欄の parameterLabel 列の非表示**、**map_DataName.csv の両綴り対応**（実出力=大文字D。CI の ubuntu で意味を持つ検証））、measure_labels（番号→dataName。dataName無しで空）、**Override 候補も実データ由来**（評価側の測定が無いデータでは [False] のみ — [False, True] ハードコード廃止の回帰。fixture は conftest の `data_dir_mini_no_override_true`、2026-07-31）、**.csv.gz だけのディレクトリでも type・カタログ・Measure ラベルが出る**（エンジンと UI の非対称の回帰 — 2026-07-31） |
| `test_dummy.py` | 単体 | ダミー一式の Board/Chip 複製展開（scorelib_param/dummy.py）。行数・番号付け・Boardごとに違うChip数・initial_temperature/map の扱い・複数Board元データの拒否（温度ファイル単独の複数 Board も同様）。**initial_temperature は実機形式（ヘッダあり）でも形式を保って展開・読み戻せる**（展開後の全文一致 + load_board_temperatures — 「could not convert string to float: 'Board'」の回帰、2026-08-03。make_pseudo_dummy 側の形式保存も同様に全文一致で検証）。**「mean 集計は複製に対して不変」という性質**で「展開は行の複製だけ」を検証し、展開一式で実計算が通ることも確認 |
...
| `test_wlgroup_keys.py` | 単体 | WLgroup 定義の在り処の一本化（0.7.0）: エクスポートが WL 軸の WLgroup / WLgroupWeight を WLgroup 系キー（実験スクリプト形式）だけに書くこと（groupDefs に残らない・WL 以外の定義は残る）、エクスポート→インポートのラウンドトリップ、ScoreFile の WLgroup 系キー吸収（groupDefs 優先・不正な文字列 bool の拒否）、to_score_file の統合、"WLgroup" 予約名ガード |
