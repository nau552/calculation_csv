...
§2〜8 は 2026-07-21 時点のスナップショット。以降の変更は §9 の追記と
...
17. **リポジトリ整理+日本語化**（ユーザーと合意の上で実施）:
   ...
   → **ここまで完了**。本セッション（2026-07-21）はここで終了。
...
（2026-07-21〜27 の間のセッション記録は本ファイルに追記されていない。この間の
...
- 追記(2026-07-31): 品質向上パスの仕上げ2点(ユーザーと相談の上)。①残す例外
  ...
  してコミット。
- 追記(2026-07-31): エラー表示のユーザー報告(「filter の値がデータに無い
  ...
  当時の説明は誤診 — 真因は次の追記を参照。
- 追記(2026-07-31): 実機の「⚠ が付かないのに dVtBudget_R2A で null エラー」の
...
- 追記(2026-07-31): 上記の候補修正の副作用をユーザーが実機で発見 —「画面2を
...
- 追記(2026-07-31): 実機報告「パーツを改名すると D&D 一覧は追随するが
...
- 追記(2026-07-31): エラー表示の用語と落ち方をユーザーと議論して一本化。
  ...
  版上げ対象(CHANGELOG 記載)。
- 追記(2026-07-31): 実機報告「元に戻すが一部の操作でしか効かない」。調査:
...
## 2026-07-31(続き): 処理フロー図解の作成と、そこで見つかった潜在不具合の一掃

- ユーザー依頼「どこから何が呼ばれてどう処理されるかを HTML で網羅的に図解」。
  エンジン・UI・バッチの3系統を並列調査(行番号つき呼び出しチェーン)し、
  13ケースの mermaid フロー図 + 検証機能の対応表を `docs/flow_guide.html` に
  まとめた(Claude の Artifact としても公開。図はリポジトリ版では mermaid を
  CDN から読むため、オフラインでは図がテキスト表示になる旨をファイル冒頭に注記)。
- 図解のためのコード全読で見つけた潜在不具合を修正(ユーザー指示「一通り修正」):
  1. axis_resolve: dataName_* 欠落時に FileNotFoundError がパーツ名の名指しなしで
     漏れる → 軸名つき ValueError に(エンジン変更)
  2. introspect: .csv.gz を読めない(エンジンと UI の非対称) → data_file 経由に
     (エンジン変更)
  3. app: データに無い type の「雛形を再生成」で KeyError → 無効化
  4. widgets: 空カタログの「+ 事前集計を追加」で min() 例外 → 無効化
  5. app: 画面3・4だけの編集が下書き自動保存されない → 保存対象キーを追加
  6. state: 複合軸の split("&") リテラルを COMBINED_SEP に統一(挙動不変)
- ドキュメント追随: code_reference(9箇所: --generation-info 追記・undo の
  廃止済み記述の削除・gz 対応・各シグネチャ差分ほか)、testing_guide、
  batch_design に「実装との差分メモ」節を追記、CHANGELOG 未リリース節を新設。
- テスト5本追加(dataName 欠落エラー・gz 検出・ボタン無効化2種・画面3の
  下書き保存)、計342件パス。ruff クリーン。
- 版数判断: エンジン変更(1・2)は不具合修正で config 語彙は不変 → 版上げは
  次リリース時に最後の数字(CHANGELOG 未リリース節に記録済み)。UI 変更は
  版数を動かさない(次の ui-YYYYMMDD の材料)。
- 追記(2026-08-03): 処理フロー図解(docs/flow_guide.html)へのユーザー質問3件から
  用語と互換コードの整理に発展。①ケース1bのダミー分岐条件を3条件の箇条書きに
  書き直し。②「旧形式キー」という呼称は誤りと指摘(WLgroup /
  WLgroupDefinLogical / WLgroupWeight は実験スクリプトが読む config の正式な
  形式で、「新形式」なるものは存在しない — groupDefs / weightSets はそれを
  任意軸へ一般化した内部の入れ物にすぎない)→ コード内コメント・docs・CHANGELOG
  0.7.0 節・テスト名(test_wlgroup_legacy.py → test_wlgroup_keys.py)まで
  「WLgroup 系キー」に全置換。③「暗黙集約の正体: group_keys」という種明かし調の
  見出しを「order に書かない軸の扱い」に平文化。さらに「values を使う機会が
  あるのか」の追及から、リリース実績が無い(=互換ガード不要)ことを確認し、
  開発初期の自前表記の互換コードを一掃: values 別名・*_subset op・
  group_reduce 案内エラー・relative.enabled ガードを対応テスト・testing_guide の
  監査メモ(「現行スクリプトがまだ使う」は誤記で根拠なし)ごと削除。
  groupDefs の存在意義への質問には「WL 以外の軸(STR 等)への一般化のため・
  設定に書かなければ現れない」と回答し現状維持で合意。
- 追記(2026-08-03): 文書の使い分けをユーザーが明示 — 設計書には過去の記録が
  あってよいが、ユーザに見せる可能性のある文書は余計な記述をなくす。これに
  従い README から存在しないものへの言及を一掃(group_reduce 廃止案内・
  enabled フラグの注記・「旧仕様の」「従来通り」等の限定句 計8箇所)。
  flow_guide.html の動作説明2箇所も同様(付録の日付つき修正記録は記録として
  維持)。設計書(design/ui_design)の group_reduce 廃止経緯の記録は残す。
- 追記(2026-08-03): リリース準備としてコードベース全体の品質監査(エンジン・UI・
  ファイル残骸の3方向並列)。結論は「デバッグ残骸・TODO・コメントアウトはゼロ、
  実質的な無駄は少数」。実施した削除・一本化は CHANGELOG「未リリース」参照。
  監査で「消してはいけない見た目未使用」も確定: vthSkip.epochs と
  constraintThreshold の中身(UI エクスポートのラウンドトリップで実験フロー側
  設定を保全するパススルー)、係数表の a(形式判別兼文書)、到達不能防御(型保証)、
  Fetcher 抽象(リモート fetcher 計画済み)。バグ疑い1件(既存スコアボタンの
  import_config_group_defs 引数欠け)は再現テストで「to_score_file 統合済みの
  ため no-op = 実害なし」と確定し、呼び出しごと削除+挙動固定テストを追加。
