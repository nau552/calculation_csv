...
- [ ] 現時点のテストが通る: `uv run pytest -q`（ベースライン確認。
...
- 変更対象: `pyproject.toml`、`uv.lock`、手順 3-4 で追随したファイルのみ
- 1コミットにまとめる。メッセージ案:

```
dev 依存を dependency-groups へ移行し uv.lock を更新(rebuild 後の uv sync --frozen だけで開発ツールが揃うように)
```
