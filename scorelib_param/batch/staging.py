...
from scorelib_param import axis_resolve
...
@dataclass
class StagedEpoch:
    """計算に渡せる状態になった 1 epoch。

    error が入っている場合は計算せずスキップ対象(skip-and-report)。
    """

    ref: EpochRef
    data_dir: Path  # エンジンに渡すディレクトリ(元 or ビュー)
    created_dir: Path | None = None  # ステージングとして作った場合のみ(削除対象)
    error: str | None = None
...
def _flatten_single_dir(view: Path) -> None:
    """`tar czf x.tar.gz result.0001/` とディレクトリごと固められていた場合の救済。

    ビュー直下が「1ディレクトリのみ」なら中身を持ち上げる。
    """
...
def validate_epoch(staged: StagedEpoch, required_types: Sequence[str], *, needs_dvtbudget: bool) -> str | None:
    ...
    if needs_dvtbudget and not axis_resolve.data_file(staged.data_dir, "initial_temperature.csv").exists():
