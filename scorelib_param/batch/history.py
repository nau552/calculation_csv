...
# 非 epoch エントリの警告に列挙する最大件数(残りは "..." で省略)
_MAX_IGNORED_LISTED = 10
...
def derive_label(history_path: str | Path) -> str:
    """result_history のパスからデフォルトのラベル `{実験ログ名}/Step{N}/Loop{NN}` を導出する(親を3段さかのぼる)。
...
def enumerate_epochs(
    ...
    """result_history のリスト(または {ラベル: パス} 辞書)から全 epoch を列挙する。

    リストの場合ラベルは `derive_label` で導出する。
    ...
    for label, path in labeled.items():
        if not path.is_dir():
            msg = f"result_history not found (label '{label}'): {path}"
            raise ValueError(msg)
        found: dict[int, Path] = {}
        ignored: list[str] = []
        ...
        if not found:
            msg = f"no result.NNNN epoch directories in {path} (label '{label}')"
            raise ValueError(msg)
        refs.extend(EpochRef(label=label, epoch_no=no, source_dir=dir_) for no, dir_ in sorted(found.items()))
    return refs
