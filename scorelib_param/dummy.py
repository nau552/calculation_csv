...
def expand_boards_chips(
    src_dir: str | Path, dest_dir: str | Path, chip_counts: Sequence[int]
) -> Path:
    ...
    dest = Path(dest_dir)
    if not src.is_dir():
        raise ValueError(f"dummy bundle directory not found: {src}")
    chip_counts = [int(n) for n in chip_counts]
    if not chip_counts or any(n < 1 for n in chip_counts):
        raise ValueError(
            f"chip_counts must be one positive chip count per board, got {chip_counts}"
        )
    ...
    for f in sorted(p for p in src.iterdir() if p.is_file()):
        if f.name == "initial_temperature.csv":
            ...
            out = pl.DataFrame(
                {"Board": list(range(len(chip_counts))), "Temperature": [temp] * len(chip_counts)}
            )
        ...
        for b, n_chips in enumerate(chip_counts):
            ...
            if has_chip:
                for c in range(n_chips):
                    parts.append(
                        df.with_columns(board_lit, pl.lit(c).cast(df.schema["Chip"]).alias("Chip"))
                    )
            else:
...
def make_pseudo_dummy(src_dir: str | Path, dest_dir: str | Path) -> Path:
    """実データ一式から Board/Chip を1つ（最小番号のみ・0 に正規化）へ削った
    疑似ダミーを `dest_dir` に書き出す（開発・検証用）。"""
    src = Path(src_dir)
    dest = Path(dest_dir)
    if not src.is_dir():
        raise ValueError(f"data directory not found: {src}")
    dest.mkdir(parents=True, exist_ok=True)
