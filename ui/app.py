...
def _handle_load(
    ...
    config_in: str,
    up_zip: UploadedFile | None,
    up_dummy: UploadedFile | None,
    up_coef: UploadedFile | None,
    up_custom: UploadedFile | None,
    paths: dict[str, str],
    n_boards: int | None,
    chips_text: str | None,
) -> None:
...
def screen_data() -> None:
    ...
    if data_mode == _DATA_MODES[0]:
        ...
        chips_text = cd2.text_input(
            ...
            value="2",
            key="dummy_chips",
            help="全 Board 共通なら数1つ(例: 4)。"
            "Board ごとに違う場合はカンマ区切りで Board の数と同じ個数(例: 4,4,2,2)",
        )
        if paths_mode:
        ...
        if up is not None and st.button("取り込む", key="part_import_btn"):
            try:
                ...
                for p in imported["score_parts"]:
                    p["name"] = (
                        p["name"] if p["name"] not in state.part_names(sf) else state.unique_part_name(sf, p["name"])
                    )
