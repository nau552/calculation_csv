...
ENGINE_OPT_KEYS = (
    "score_function", "expression", "score_parts", "constraintThreshold",
    "WLgroup", "WLgroupDefinLogical", "WLgroupWeight", "weightSets",
    "selectionSets", "groupDefs", "vthSkip",
)


def _strip_jsonc(text):
    """jsonc → json の簡易変換（// と /* */ のコメント・末尾カンマを除去）。
...
def _plain(obj):
    """比較用の正規化: Series/ndarray/numpy スカラー等を素の Python 型へ
...
def report_engine_vocab_diff(config_path, processed, printer=print):
    ...
    if isinstance(processed, dict) and "optimization" in processed:
        ...
        printer("note: 渡された dict に 'optimization' キーが無いため、"
                "optimization の中身とみなして比較します")
    ...
    for scope, key, in_file, in_mem in pairs:
        ...
        printer("%-14s %-22s %s" % (scope, key, status))

    only_mem = [k for _, k, s in findings if s == "メモリのみ"]
    if only_mem:
        printer("!! ファイルに無くメモリにだけ存在するエンジン語彙: %s" % ", ".join(only_mem))
        printer("   → エンジンはファイルを読むため、これらは計算に使われません。"
                "必要な値ならファイルに書く運用にしてください")
    else:
        printer("OK: メモリにだけ存在するエンジン語彙はありません"
                "（エンジンはファイルを読めば十分）")
    return findings
